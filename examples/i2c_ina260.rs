...
fn main() {
    if let Some(p) = Peripherals::take() {
        cortex_m::interrupt::free(|cs| {
            ...
            let mut dp = unsafe { cortex_m::Peripherals::steal() };
            /* Enable USART IRQ, set prio 0 and clear any pending IRQs */
            dp.NVIC.enable(Interrupt::USART1);
            unsafe { dp.NVIC.set_priority(Interrupt::USART1, 1) };
            dp.NVIC.clear_pending(Interrupt::USART1);
...
fn read_i2c_ina260_config(i2c: &stm32f042::i2c1::RegisterBlock, address: u8) -> u16 {
    let mut data = [0; 2];
...
fn read_i2c_ina260_current(i2c: &stm32f042::i2c1::RegisterBlock, address: u8) -> u32 {
    let mut data = [0; 2];
...
fn read_i2c_ina260_voltage(i2c: &stm32f042::i2c1::RegisterBlock, address: u8) -> u32 {
    let mut data = [0; 2];
